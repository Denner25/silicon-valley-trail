...
function applyIntermediateChoice(action, choice, result, game) {
  ...
  if (action === "hackathon") {
    ...
    if (choice === "safe") {
      ...
      result.notes.push(
        "Safe approach slightly increased rewards (+20% cash)"
      );
    }
  ...
  if (action === "travel") {
    if (choice === "push") {
      ...
      result.morale -= 5;
      result.notes.push(
        "Pushed the team too hard (-20 cash, -5 morale)"
      );
    }
    ...
    if (choice === "slow") {
      result.cash -= 10;
      result.notes.push(
        "Slow travel reduced expenses (-10 cash)"
      );
    }
    ...
    if (choice === "coffee") {
      ...
      result.coffee -= 1;
      result.notes.push(
        "Coffee stop boosted morale (+1 morale, -1 coffee)"
      );
    }
  }
  ...
  if (action === "rest") {
    ...
    if (choice === "long") {
      ...
      result.coffee -= 1;
      result.notes.push(
        "Long rest restored morale (+10 morale, -1 coffee)"
      );
    }
  }
}
...
function startGame(req, res) {
  ...
  const currentLocation =
    LOCATIONS[game.locationIndex]?.name || "Unknown";
  ...
  Game.findById(gameId)
    .then((game) => {
      ...
      return Promise.all([
        ...
        fetchGitHubMarketPressure(),
        fetchHackerNewsMorale(),
      ]).then(([weatherImpact, marketData, newsData]) => {
        ...
        if (newsMorale !== 0) {
          result.notes.push(
            `Tech news affected morale by ${newsMorale}`
          );
        }
        ...
        if (
          ...
          if (game.resources.coffee <= 0) {
            ...
            game.deathNote =
              "Bankruptcy! The project ran out of funds.";
          }
        }
